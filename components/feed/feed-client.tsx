...
import { Star, Rss, List, ChevronDown, Filter } from "lucide-react";
...
interface FeedItem {
  ...
  addedAt: string;
  movie: {
    id: string; title: string; posterUrl: string | null; rating: number | null;
    releaseDate: string | null; genres: string[]; runtime: number | null; overview: string | null;
  };
  list: {
              ...
              <div className="flex gap-2 flex-wrap mb-5">
                ...
                {allGenres.map(g => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${genreFilter === g ? "bg-red-600/20 border-red-600/40 text-red-400" : "border-white/10 text-zinc-400 hover:border-white/20"}`}
                  >
                    {g}
                  </button>
                ))}
                    ...
                    <Link href={`/movies/${item.movie.id}`} className="flex gap-3 group">
                      <div className="relative h-24 w-16 rounded-lg overflow-hidden bg-zinc-800 flex-shrink-0">
                        {item.movie.posterUrl ? (
                          <Image src={item.movie.posterUrl} alt={item.movie.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" />
                        ) : (
                          <div className="flex items-center justify-center h-full text-2xl">🎬</div>
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-white group-hover:text-red-400 transition-colors leading-snug">{item.movie.title}</p>
                        <div className="flex flex-wrap items-center gap-2 mt-1">
                          {item.movie.releaseDate && <span className="text-xs text-zinc-500">{formatYear(item.movie.releaseDate)}</span>}
                          {item.movie.rating ? (
                            <span className="flex items-center gap-1 text-xs text-yellow-400">
                              <Star className="h-3 w-3 fill-yellow-400" />{formatRating(item.movie.rating)}
                            </span>
                          ) : null}
                        </div>
                        <div className="flex flex-wrap gap-1 mt-1.5">
                          {item.movie.genres.slice(0, 3).map(g => <Badge key={g} variant="secondary">{g}</Badge>)}
                        </div>
                        {item.movie.overview && <p className="text-xs text-zinc-600 mt-1.5 line-clamp-2">{item.movie.overview}</p>}
                      </div>
                    </Link>

                    {item.note && (
                      <p className="mt-3 text-sm text-zinc-400 italic border-l-2 border-red-600/30 pl-3">{item.note}</p>
                    )}
                  </div>
                </StaggerItem>
              ))}
