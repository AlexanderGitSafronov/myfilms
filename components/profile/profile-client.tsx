...
export function ProfileClient({
  ...
  const [form, setForm] = useState({ name: initialProfile.name || "", bio: initialProfile.bio || "" });
          ...
          <div className="flex-1 min-w-0">
            {editing ? (
              <form onSubmit={handleSave} className="space-y-3">
                <Input label={t("displayName")} value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder={t("displayName")} />
                <Textarea label={t("bio")} value={form.bio} onChange={(e) => setForm((f) => ({ ...f, bio: e.target.value }))} placeholder={t("bio")} rows={3} />
              ...
              <>
                ...
                <Button variant="outline" size="sm" className="mt-4" onClick={() => setEditing(true)}>{t("editProfile")}</Button>
              </>
            )}
          </div>
        </div>
          ...
          {getAchievements(stats, profile).map(({ id, icon, labelKey, descKey, unlocked }, i) => (
            <motion.div
              ...
              className={`rounded-xl border p-3 flex items-center gap-3 ${
                unlocked ? "border-yellow-500/30 bg-yellow-500/5" : "border-white/5 bg-white/[0.01] opacity-40"
              }`}
            >
        ...
        {profile.lists.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {profile.lists.map((list) => (
              <ListCard key={list.id} list={list} username={profile.username} isOwner />
            ))}
          </div>
        )}
      </div>
